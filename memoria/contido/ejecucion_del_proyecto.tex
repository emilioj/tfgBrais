...
En la implementación actual se emplea el diccionario \srccode{DICT\_6X6\_250}, que contiene 250 marcadores únicos de 6×6 bits cada uno. Este diccionario fue seleccionado por ofrecer un buen equilibrio entre:
...
\paragraph{Configuración del sistema de seguimiento.}
...
La principal adaptación requerida para la integración con Exposure Render consistió en la transformación de coordenadas entre los sistemas de referencia de ArUco y el motor de renderizado. Esta transformación se implementa en la función \srccode{getCubePoseMatrix}, que convierte los vectores de rotación y traslación proporcionados por ArUco en matrices de transformación 4×4 compatibles con el sistema de renderizado.
...
El marcador fiduciario utilizado en este proyecto se genera a partir de un diccionario predefinido de ArUco. En la implementación actual se emplea el diccionario \srccode{DICT\_6X6\_250}, que contiene 250 marcadores únicos de 6×6 bits cada uno. Este diccionario fue seleccionado por ofrecer un buen equilibrio entre:
...
\begin{center}
\srccode{firstId = cara\_id * 4}\\
\srccode{ids = \{firstId, firstId + 1, firstId + 2, firstId + 3\}}
\end{center}

Donde \srccode{cara\_id} corresponde al número de cara del cubo (0-5). De esta forma:
\begin{itemize}
...
La integración del sistema de tracking ArUco en Exposure Render se realizó como una librería independiente que proporciona la funcionalidad de seguimiento de marcadores fiduciarios. El punto central de esta integración es la función \srccode{getCubePoseMatrix}, que actúa como interfaz principal entre el sistema de tracking y la aplicación de realidad aumentada.

\subsubsection{Función getCubePoseMatrix}
La función \srccode{getCubePoseMatrix} es la interfaz principal que proporciona la pose del cubo marcador en el espacio tridimensional. Los parámetros de esta función incluyen configuraciones para visualización, dimensiones físicas del marcador, rutas de calibración y directorios de configuración.
...
Para minimizar la latencia y mantener la fluidez del renderizado principal, el sistema de tracking se implementó como un hilo independiente mediante la clase \srccode{Tracking} que encapsula toda la funcionalidad de seguimiento.

El flujo de ejecución del tracking opera en un hilo separado (\srccode{poseThread}) que ejecuta continuamente el siguiente proceso:
...
\begin{enumerate}
    ...
    \item \textbf{Procesamiento de marcadores}: Ejecuta la detección y seguimiento de marcadores ArUco llamando a \srccode{getCubePoseMatrix}.
...
\begin{itemize}
    \item \textbf{std::mutex}: Se utiliza para la protección del acceso a
      las variables \srccode{\_latestPose} y \srccode{\_latestCameraImage}.
    \item \textbf{std::atomic<bool>}: Se usa para controlar el estado de ejecución del hilo mediante \srccode{\_poseThreadRunning}.
...
Para facilitar la integración con Exposure Render, la librería de tracking utiliza la estructura \srccode{PoseMatrix4x4}, que representa una matriz de 4×4. Esta estructura encapsula tanto la rotación como la traslación del marcador en el espacio 3D.

La función \srccode{getCubePoseMatrix} implementa una serie de transformaciones para que la pose del cubo detectada por ArUco sea directamente utilizable por el motor de renderizado de Exposure Render. El proceso incluye:
...
\begin{enumerate}
    ...
    \item \textbf{Creación del hilo de captura}: Se inicia un hilo independiente para el \gls{tracking} de marcadores mediante \srccode{captureThreadFunc}.
